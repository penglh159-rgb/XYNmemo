...
const PREDEFINED_COLORS = [
  '#EF4444', '#F97316', '#F59E0B', '#10B981', '#3B82F6', '#6366F1', '#EC4899', '#8B5CF6', '#64748B'
];
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 pl-10">
            {PREDEFINED_COLORS.map(c => (
              <button
                ...
                className={cn(
                  "w-5 h-5 rounded-full shrink-0 shadow-sm transition-transform hover:scale-110",
                  editColor === c ? "ring-2 ring-offset-1 ring-blue-500" : "border border-slate-200"
                )}
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-3 border-b border-slate-100 bg-slate-50/50 flex flex-col gap-3">
          <div className="flex gap-2 items-center">
            <div className="relative w-8 h-8 rounded-full overflow-hidden border-2 border-slate-300 shadow-sm shrink-0 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500" style={{ backgroundColor: newColor }}>
              <input
                type="color"
                value={newColor}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 pl-10">
            {PREDEFINED_COLORS.map(c => (
              <button
                ...
                className={cn(
                  "w-5 h-5 rounded-full shrink-0 shadow-sm transition-transform hover:scale-110",
                  newColor === c ? "ring-2 ring-offset-1 ring-blue-500" : "border border-slate-200"
                )}
                style={{ backgroundColor: c }}
              />
            ))}
